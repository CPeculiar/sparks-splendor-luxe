  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        ...
        <button onClick={() => setEditing({ name: "", is_active: true })} className="inline-flex items-center gap-2 bg-onyx text-cream px-4 py-2 text-xs uppercase">
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setEditing(null)}>
          ...
          <form onSubmit={handleCreateOrUpdate} className="relative bg-background w-full max-w-lg p-6" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-display text-2xl mb-2">{editing.id ? "Edit Category" : "New Category"}</h2>
            <input required className="inp mb-2" value={editing.name || ""} onChange={(e) => setEditing({ ...editing, name: e.target.value })} placeholder="Name" />
            <textarea className="inp mb-2" value={editing.description || ""} onChange={(e) => setEditing({ ...editing, description: e.target.value })} placeholder="Description" />
            <CloudinaryUpload label="Upload image" onUpload={(url) => setEditing({ ...editing, image_url: url })} />
            <div className="pt-4 flex gap-3">
              <button type="button" onClick={() => setEditing(null)} className="flex-1 border border-border py-3">Cancel</button>
              <button type="submit" className="flex-1 bg-onyx text-cream py-3">Save</button>
            </div>
          </form>
        </div>
      )}
